    ...
    public class FileMapping : IDisposable
        ...
        public FileMapping(String filename)
            ...
            try
                ...
                Size = output.Size;
                #else
                using (var file = CreateFile(filename, FileGenericAccess.Read, FileShare.Read, null, FileMode.Open, 0, IntPtr.Zero))
                    {
                    if (file.IsInvalid) { throw new Win32Exception(Marshal.GetLastWin32Error(), filename); }
        ...
        [DllImport("kernel32.dll", BestFitMapping = false, CharSet = CharSet.Auto, SetLastError = true)] private static extern SafeFileHandle CreateFile(String filename, FileGenericAccess desiredaccess, FileShare dwShareMode, SecurityAttributes security, FileMode creationdisposition, FileAttributes flags, IntPtr templatefile);
