...
namespace BinaryStudio.Security.Cryptography.CryptographyServiceProvider
    {
    [AlgorithmIdentifer(ObjectIdentifiers.szOID_NIST_sha384)]
    internal class OpenSSLSHA384 : OpenSSLHashAlgorithm
        {
        private SHA512_CTX context;
        private Boolean ready;

        /// <inheritdoc/>
        public OpenSSLSHA384(OpenSSLCryptographicContext context)
            : base(context)
            {
            HashSizeValue = 384;
            }
        ...
        private void Ensure() {
            if (!ready) {
                Context.Core.SHA384_Init(ref context);
                ready = true;
                }
            }
        ...
        protected override unsafe void HashCore(Byte[] data, Int32 firstindex, Int32 length) {
            Ensure();
            fixed (Byte* r = data) {
                Context.Core.SHA384_Update(ref context, r + firstindex, (IntPtr)length);
                }
            }
        ...
        protected override Byte[] HashFinal()
            ...
            Context.Core.SHA384_Final(r, ref context);
            return r;
            }
        }
    }