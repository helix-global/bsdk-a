...
namespace BinaryStudio.Security.Cryptography.Xml {
    ...
    internal static class SignedXmlDebugLog {
        ...
        private static String GetKeyName(Object key) {
            ...
            var cspKey = key as ICspAsymmetricAlgorithm;
            var certificate = key as X509Certificate;
            var certificate2 = key as X509Certificate2;
        ...
        internal static void LogBeginCanonicalization(SignedXml signedXml, Transform canonicalizationTransform) {
            ...
            if (VerboseLoggingEnabled) {
                var canonicalizationSettings = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogBeginCheckSignatureFormat(SignedXml signedXml, Func<SignedXml, Boolean> formatValidator) {
            ...
            if (InformationLoggingEnabled) {
                var validationMethod = formatValidator.Method;
        ...
        internal static void LogBeginSignatureComputation(SignedXml signedXml, XmlElement context) {
            ...
            if (VerboseLoggingEnabled) {
                var contextData = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogCanonicalizedOutput(SignedXml signedXml, Transform canonicalizationTransform) {
            ...
            if (VerboseLoggingEnabled) {
                using (var reader = new StreamReader(canonicalizationTransform.GetOutput(typeof(Stream)) as Stream)) {
                    var logMessage = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogFormatValidationResult(SignedXml signedXml, Boolean result) {
            ...
            if (InformationLoggingEnabled) {
                var logMessage = result ? SecurityResources.GetResourceString("Log_FormatValidationSuccessful") :
        ...
        internal static void LogUnsafeCanonicalizationMethod(SignedXml signedXml, String algorithm, IEnumerable<String> validAlgorithms) {
            ...
            if (InformationLoggingEnabled) {
                var validAlgorithmBuilder = new StringBuilder();
            ...
            IEnumerable<String> validTransformAlgorithms)
        {
            ...
            if (InformationLoggingEnabled) {
                var validAlgorithmBuilder = new StringBuilder();
                foreach (var validAlgorithm in validC14nAlgorithms) {
                    if (validAlgorithmBuilder.Length != 0) {
                        validAlgorithmBuilder.Append(", ");
                    }
                ...
                foreach (var validAlgorithm in validTransformAlgorithms) {
                    if (validAlgorithmBuilder.Length != 0) {
                        validAlgorithmBuilder.Append(", ");
                    }
            ...
            if (InformationLoggingEnabled) {
                if (namespaces != null) {
                    foreach (XmlAttribute propagatedNamespace in namespaces) {
                        var propagationMessage = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogSigningReference(SignedXml signedXml, Reference reference) {
            ...
            if (VerboseLoggingEnabled) {
                var hashAlgorithm = Utils.CreateFromName<HashAlgorithm>(reference.DigestMethod);
                var hashAlgorithmName = hashAlgorithm == null ? "null" : hashAlgorithm.GetType().Name;
                var logMessage = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogVerificationResult(SignedXml signedXml, Object key, Boolean verified) {
            ...
            if (InformationLoggingEnabled) {
                var resource = verified ? SecurityResources.GetResourceString("Log_VerificationWithKeySuccessful") :
        ...
        internal static void LogVerifySignedInfo(SignedXml signedXml,
            ...
            if (VerboseLoggingEnabled) {
                var hashLog = String.Format(CultureInfo.InvariantCulture,
                ...
                var signatureLog = String.Format(CultureInfo.InvariantCulture,
        ...
        internal static void LogVerifyX509Chain(SignedXml signedXml, X509Chain chain, X509Certificate certificate) {
            ...
            if (InformationLoggingEnabled) {
                var buildMessage = String.Format(CultureInfo.InvariantCulture,
            ...
            if (VerboseLoggingEnabled) {
                ...
                var revocationMode = String.Format(CultureInfo.InvariantCulture,
                ...
                var revocationFlag = String.Format(CultureInfo.InvariantCulture,
                ...
                var verificationFlags = String.Format(CultureInfo.InvariantCulture,
                ...
                var verificationTime = String.Format(CultureInfo.InvariantCulture,
                ...
                var urlTimeout = String.Format(CultureInfo.InvariantCulture,
            ...
            if (VerboseLoggingEnabled) {                
                var chainElements = new StringBuilder();
